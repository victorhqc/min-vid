...
function DraggableElement(elem) {
  this._elem = elem;
  this._window = elem instanceof Ci.nsIDOMChromeWindow ? elem : elem.ownerGlobal;
...
function handleEvent(aEvent) {
  if (aEvent.type === 'mousedown' && /^gtk/i.test(AppConstants.MOZ_WIDGET_TOOLKIT)) {
    ...
    const winPtrStr = nativeWindowUtils.getNativeHandlePtrStr(this._window); // eslint-disable-line no-undef
    ...
    const button = 1;
    const coords = nativeWindowUtils.getMouseInfo(); // eslint-disable-line no-undef
...
const EXPORTED_SYMBOLS = ['DraggableElement']; // eslint-disable-line no-unused-vars
