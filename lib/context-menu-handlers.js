...
self.on('click', function (node, data) {
  console.log('contextMenuMediacontentscript', self, node, data);
  let host = '';
  try {
  ...
  self.postMessage({
    ...
    domain: host // new URL(node.href || node.src).host
  });
});`;
...
function init() {
  ...
  getItem({
    ...
    context: [
      cm.URLContext(['*.youtube.com']),
      cm.SelectorContext('.yt-lockup-thumbnail > .yt-uix.sessionlink .video-thumb').parentNode
    ],
    contentScript: contextMenuContentScript,
    onMessage: onMessageYouTubeHandler
  });
